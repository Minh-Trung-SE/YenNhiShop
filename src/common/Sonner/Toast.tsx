...
import TRIGGER_TOAST_TYPE from "@src/common/Sonner/enum";
    ...
    return (
        ...
        >
            <div className="flex items-center space-x-1">
                <div className={twMerge("flex-none h-5 w-5", variants[type])}>
                    {ICONS[type]}
                </div>
                <div className="grow flex items-center">
                    {
                        isString(header) ? (
                            <h2 className={twMerge("grow font-medium", variants[type])}>{header}</h2>
                        ) : (
